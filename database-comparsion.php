...
<form method="POST">
    ...
    <p><input type="checkbox" name="exec" value="1" <?php echo ($_REQUEST['exec'] == 1) ? 'checked' : '' ?> >执行SQL更新操作
    </p>
</form>
...
$tCreate = [];
$tIndex  = [];
foreach([$_REQUEST['db1'] => 1, $_REQUEST['db2'] => 2] as $db => $linkNo) {
    $link = getDB($linkNo);
